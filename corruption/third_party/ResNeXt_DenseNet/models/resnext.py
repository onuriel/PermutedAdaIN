...
class ResNeXtBottleneck(nn.Module):
  ...
  def __init__(self,
               ...
               base_width,
               stride=1,
               downsample=None, with_permute_adain=False, p_adain=0):
  ...
  def forward(self, x):
    ...
    if self.with_permute_adain:
        bottleneck = self.perumte_adain(bottleneck)
...
class CifarResNeXt(nn.Module):
  ...
  def __init__(self, block, depth, cardinality, base_width, num_classes, with_permute_adain=False, p_adain=0):
  ...
  def _make_layer(self, block, planes, blocks, stride=1):
    ...
    layers.append(
        block(self.inplanes, planes, self.cardinality, self.base_width, stride,
              downsample, with_permute_adain=self.with_permute_adain, p_adain=self.p_adain))
    self.inplanes = planes * block.expansion
    for _ in range(1, blocks):
      layers.append(
          block(self.inplanes, planes, self.cardinality, self.base_width, with_permute_adain=self.with_permute_adain, p_adain=self.p_adain))
...
def resnext29(num_classes=10, cardinality=4, base_width=32, with_permute_adain=False, p_adain=0):
  model = CifarResNeXt(ResNeXtBottleneck, 29, cardinality, base_width,
                       num_classes, with_permute_adain=with_permute_adain, p_adain=p_adain)
  return model
