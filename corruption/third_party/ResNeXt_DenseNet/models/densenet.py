...
from architectures.permuteAdaIN import PermuteAdaptiveInstanceNorm2d


class Bottleneck(nn.Module):
  """Bottleneck block for DenseNet."""

  def __init__(self, n_channels, growth_rate, with_permute_adain=False, p_adain=0):
...
class Transition(nn.Module):
  ...
  def __init__(self, n_channels, n_out_channels, with_permute_adain=False, p_adain=0):
...
class DenseNet(nn.Module):
  ...
  def __init__(self, growth_rate, depth, reduction, n_classes, bottleneck, with_permute_adain=False, p_adain=0):
  ...
  def _make_dense(self, n_channels, growth_rate, n_dense_blocks, bottleneck):
    ...
    for _ in range(int(n_dense_blocks)):
      if bottleneck:
        layers.append(Bottleneck(n_channels, growth_rate, with_permute_adain=self.with_permute_adain, p_adain=self.p_adain))
      else:
        layers.append(SingleLayer(n_channels, growth_rate, with_permute_adain=self.with_permute_adain, p_adain=self.p_adain))
...
def densenet(growth_rate=12, depth=40, num_classes=10, with_permute_adain=False, p_adain=0):
  model = DenseNet(growth_rate, depth, 1., num_classes, False,  with_permute_adain=with_permute_adain, p_adain=p_adain)
  return model
